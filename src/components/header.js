...
export default class Header extends React.Component {
                ...
                <LinearGradient colors={['#3867D5', '#81C7F5']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ height: "100%", display: "flex", alignItems: "center", flexDirection: "column-reverse" }}>
                    ...
                    <View style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", alignContent: "center", width:"90%", marginBottom:10}}>
                        <View style={{display: "flex", flexDirection: "column", justifyContent: "space-around",}}>
                            <Text style={{fontSize:17, color:"white"}}>Hello {user.first_name}!</Text>
    ...
    getBackgroundBubbles() {
        return <View style={{ position: 'absolute', width: "100%", display: "flex", flexDirection: "row", justifyContent: "space-between" }}>
    ...
    getTodaysReminders() {
        ...
        if (todaysReminderCount > 0) {
            return <Text style={{ fontSize: 10, color: "white" }}>Today you have {todaysReminderCount} reminders</Text>;
        }
        return <Text style={{ fontSize: 10, color: "white" }}>Today you have no reminders</Text>;
    }
}
